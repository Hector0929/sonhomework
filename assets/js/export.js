...
export function initExport(win, doc){
  ...
  dlTxt && !dlTxt.__wired && (dlTxt.addEventListener('click', ()=>{ g.refreshExportPreview(); const box=d.getElementById('export-preview-text'); const text=(box?box.value:gatherCurrentContent()||'').replace(/\r?\n/g,'\r\n'); const blob=new Blob([text],{type:'text/plain;charset=utf-8'}); const a=d.createElement('a'); const title=(d.getElementById('song-title')?.value||'chords').replace(/\s+/g,'_'); const key=(d.getElementById('to-key')?.value||d.getElementById('from-key')?.value||'C'); a.href=URL.createObjectURL(blob); a.download=`${title}_${key}_export.txt`; a.style.display='none'; d.body.appendChild(a); a.click(); setTimeout(()=>{ URL.revokeObjectURL(a.href); d.body.removeChild(a); },1000); }), dlTxt.__wired=true);
